  ...
  function endedView(state, opts) {
    ...
    return `
      <div class="kal-ended">
        ...
        ${state.reward ? `<div class="kal-reward"><img class="shadow-coin-icon" src="assets/ui/shadow-coin.webp" alt="Shadow Coin"><div><b>+${Number(state.reward.amount) || 1} SHADOW COIN</b>${Number.isFinite(state.reward.balance) ? `<small>SHADOW COINS: ${Number.isInteger(state.reward.balance) ? state.reward.balance : state.reward.balance.toFixed(1)}</small>` : ''}</div></div>` : ''}
